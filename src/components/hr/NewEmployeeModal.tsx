    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
        <motion.div
          ...
          className="bg-white dark:bg-[#0C0C0D] border border-black/[0.06] dark:border-white/[0.08] rounded-2xl max-w-xl w-full shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
        >
          {/* Header */}
          <div className="p-5 border-b border-black/[0.05] dark:border-white/[0.08] flex items-center justify-between bg-slate-50/50 dark:bg-[#121214]">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-[#A39E93]">Provision active staff member & baseline payroll docket</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  className="w-full text-xs px-3.5 py-2.5 rounded-xl border border-slate-300 dark:border-white/[0.1] bg-white dark:bg-[#141416] text-slate-900 dark:text-[#F6F4F0] placeholder:text-slate-400 dark:placeholder:text-[#8E8E93] focus:outline-hidden focus:ring-2 focus:ring-emerald-500/30"
                />
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Job Title / Designation</label>
                <input
                  type="text"
                  required
                  value={jobTitle}
                  onChange={e => setJobTitle(e.target.value)}
                  className="w-full text-xs px-3.5 py-2 rounded-xl border border-slate-300 dark:border-white/[0.1] bg-white dark:bg-[#141416] text-slate-900 dark:text-[#F6F4F0]"
                />
              </div>

              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Department</label>
                <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Functional Role</label>
                <select
                  value={functionalRole}
                  onChange={e => setFunctionalRole(e.target.value as FunctionalRole)}
                  className="w-full text-xs px-2.5 py-2 rounded-xl border border-slate-300 dark:border-white/[0.1] bg-white dark:bg-[#141416] text-slate-900 dark:text-[#F6F4F0]"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Access Tier</label>
                <select
                  value={accessTier}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Management Tier</label>
                <select
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Direct Line Manager</label>
                <select
                  value={managerId}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Phone Number</label>
                <input
                  type="text"
                  value={phone}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-slate-700 dark:text-[#D1CDC7]">Technical Skills (comma-separated)</label>
              <input
                type="text"
                value={skills}
            ...
            <div className="pt-4 border-t border-black/[0.05] dark:border-white/[0.08] flex items-center justify-end gap-2.5">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 px-5 py-2 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-500 active:scale-95 rounded-xl shadow-xs transition-all disabled:opacity-50 cursor-pointer"
              >
