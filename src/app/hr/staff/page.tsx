...
export default function HRStaffPage() {
  ...
  const handleExportStaffPdf = () => {
    exportToPdf({
      filename: `AquaEarth_Staff_Nominal_Roll_${new Date().toISOString().split('T')[0]}`,
      title: 'Human Capital Nominal Roll & Workforce Competency Audit',
      subtitle: 'AquaEarth Consulting Limited — Official Personnel Directory & Regulatory Audit',
      category: 'HUMAN RESOURCES AUDIT',
      summaryMetrics: [
        { label: 'Total Headcount', value: String(allUsers.length) },
        { label: 'Active Staff', value: String(allUsers.filter(u => u.status === 'ACTIVE').length) },
        { label: 'Departments', value: String(new Set(allUsers.map(u => u.departmentName)).size) },
        { label: 'Leadership / PMs', value: String(allUsers.filter(u => u.managementTier !== 'NONE' || u.functionalRole === 'PROJECT_MANAGER').length) }
      ],
      columns: [
        { header: 'Employee ID', key: 'id', width: '80px' },
        { header: 'Full Name', key: 'name' },
        { header: 'Job Title', key: 'jobTitle' },
        { header: 'Department', key: 'departmentName', format: (val) => val || 'Operations' },
        { header: 'Functional Role', key: 'functionalRole' },
        { header: 'Mgmt Tier', key: 'managementTier', format: (val) => val || 'NONE' },
        { header: 'Manager / Lead', key: 'managerName', format: (val) => val || 'Direct MD' },
        { header: 'Status', key: 'status' }
      ],
      data: allUsers,
      signatories: [
        { role: 'HEAD OF HR & ADMIN', name: 'Mrs. Funke Balogun', title: 'HR & Administrative Lead' },
        { role: 'CHIEF OPERATING OFFICER', name: 'Engr. Femi Adebayo', title: 'Head of Operations' },
  ...
  const handleExportStaffXls = () => {
    exportToXls({
      filename: `AquaEarth_Staff_Nominal_Roll_${new Date().toISOString().split('T')[0]}`,
      title: 'HUMAN CAPITAL NOMINAL ROLL & WORKFORCE AUDIT',
      subtitle: `Official Records | Generated by: ${currentUser.name}`,
      category: 'HR AUDIT',
      metadata: {
        'Custodian': currentUser.name,
        'Active Count': String(allUsers.filter(u => u.status === 'ACTIVE').length),
        'Total Headcount': String(allUsers.length)
      },
      summaryMetrics: [
        { label: 'Total Headcount', value: allUsers.length },
        { label: 'Active Staff', value: allUsers.filter(u => u.status === 'ACTIVE').length }
      ],
      columns: [
        ...
        { header: 'Department', key: 'departmentName', format: (val) => val || 'Operations' },
        { header: 'Functional Role', key: 'functionalRole' },
        { header: 'Access Tier', key: 'accessTier' },
        { header: 'Management Tier', key: 'managementTier' },
        { header: 'Reporting Manager', key: 'managerName', format: (val) => val || 'Direct MD' },
        { header: 'Phone', key: 'phone', format: (val) => val || '' },
        { header: 'Location', key: 'location', format: (val) => val || 'Lagos HQ' },
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              <button
                type="button"
                onClick={handleExportStaffPdf}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white dark:bg-[#141416] hover:bg-slate-100 dark:hover:bg-[#1C1C1F] text-slate-700 dark:text-[#F6F4F0] text-[11px] font-semibold border border-slate-200 dark:border-white/[0.08] shadow-2xs transition-all active:scale-[0.98] cursor-pointer whitespace-nowrap shrink-0"
                title="Download PDF Nominal Roll Audit"
              >
              ...
              <button
                ...
                onClick={handleExportStaffXls}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-xl bg-white dark:bg-[#141416] hover:bg-slate-100 dark:hover:bg-[#1C1C1F] text-slate-700 dark:text-[#F6F4F0] text-[11px] font-semibold border border-slate-200 dark:border-white/[0.08] shadow-2xs transition-all active:scale-[0.98] cursor-pointer whitespace-nowrap shrink-0"
                title="Download Excel Nominal Roll"
              >
