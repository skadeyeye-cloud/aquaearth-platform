...
export default function OnboardingPipelinePage() {
  ...
  const handleExportOnboardingPdf = () => {
    exportToPdf({
      filename: `AquaEarth_Onboarding_Pipeline_${new Date().toISOString().split('T')[0]}`,
      title: 'Talent Acquisition & Candidate Onboarding Pipeline Audit',
      subtitle: `AquaEarth Consulting Limited — Stage Filter: ${filterStage} | Generated by: ${currentUser.name}`,
      category: 'TALENT ACQUISITION AUDIT',
      summaryMetrics: [
        { label: 'Active Pipeline', value: String(activeCandidates.length) },
        { label: 'In Interviews', value: String(activeCandidates.filter(c => c.currentStage.includes('INTERVIEW')).length) },
        { label: 'Offers / Probation', value: String(activeCandidates.filter(c => c.currentStage === 'PROBATIONARY' || c.currentStage === 'FULL_EMPLOYMENT').length) },
        { label: 'Total Applicants', value: String(candidateApplications.length) }
      ],
      columns: [
        { header: 'Docket No', key: 'candidateNumber', width: '90px' },
        { header: 'Candidate Name', key: 'fullName' },
        { header: 'Target Role', key: 'appliedRole' },
        { header: 'Department', key: 'department' },
        { header: 'Exp (Yrs)', key: 'yearsExperience', align: 'center', format: (val) => `${val} yrs` },
        { header: 'Expected Salary', key: 'expectedSalaryNgn', align: 'right', format: (val) => `₦${Number(val || 0).toLocaleString()}` },
        { header: 'Current Stage', key: 'currentStage', format: (val) => STAGES.find(s => s.id === val)?.label || val }
      ],
      data: activeCandidates,
      signatories: [
        { role: 'TALENT ACQUISITION LEAD', name: currentUser.name, title: `${currentUser.jobTitle || 'Recruitment Officer'}` },
  ...
  const handleExportOnboardingXls = () => {
    exportToXls({
      filename: `AquaEarth_Onboarding_Pipeline_${new Date().toISOString().split('T')[0]}`,
      title: 'TALENT ACQUISITION & CANDIDATE ONBOARDING PIPELINE AUDIT',
      subtitle: `AquaEarth Consulting Limited — Stage Filter: ${filterStage} | Generated by: ${currentUser.name}`,
      ...
      metadata: {
        ...
        'Filter': filterStage,
        'Total Candidates': String(activeCandidates.length)
      },
      summaryMetrics: [
        { label: 'Active Pipeline', value: activeCandidates.length },
        { label: 'Total Records', value: candidateApplications.length }
      ],
      columns: [
        { header: 'Candidate Docket', key: 'candidateNumber' },
        { header: 'Candidate Full Name', key: 'fullName' },
        { header: 'Email', key: 'email' },
        { header: 'Phone', key: 'phone' },
        { header: 'Applied Role', key: 'appliedRole' },
        { header: 'Department', key: 'department' },
        { header: 'Years Experience', key: 'yearsExperience', align: 'center', format: (val) => val || 0 },
        { header: 'Expected Salary (NGN)', key: 'expectedSalaryNgn', align: 'right', format: (val) => val || 0 },
        { header: 'Current Stage', key: 'currentStage', format: (val) => STAGES.find(s => s.id === val)?.label || val }
      ],
      data: activeCandidates
    });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button
            ...
            title="Download PDF Recruitment Pipeline Audit"
          >
          ...
          <button
            onClick={handleExportOnboardingXls}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-semibold shadow-2xs transition-all active:scale-[0.96] cursor-pointer whitespace-nowrap shrink-0"
            title="Download Excel Candidate Records"
          >
